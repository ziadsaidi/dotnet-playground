    ...
    ) : ControllerBase
{
  ...
  {
    var result = await _mediator.Send(new GetAllCustomersQuery(), CancellationToken.None);
  ...
  {
    var result = await _mediator.Send(new GetCustomerByIdQuery(id), CancellationToken.None);
